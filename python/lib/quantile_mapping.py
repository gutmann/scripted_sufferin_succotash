...
def linterp(input_data, match, datacol=0, weightcol=1):
    """linearly interpolate between input_data[0,datacol] and input_data[1,datacol]
        interpolation is based on where match falls relative to the values in input_data[:,weightcol]
        """
    weight = (input_data[0,weightcol]-match) / (input_data[1,weightcol] - input_data[0,weightcol])
    return weight*input_data[0,datacol] + (1-weight)*input_data[1,datacol]


def lookup_data(data, quantile_data, datacol=0, weightcol=1):
    """look up where data falls in quantile_data and linearly interpolate to get an improved value"""
...
def map_data_with_quantiles(inputdata, input_quantile_data, output_quantile_data):
    ...
    for i in range(len(inputdata)):
        ...
        quantile = lookup_data(inputdata[i], input_quantile_data,  weightcol=0, datacol=1)
