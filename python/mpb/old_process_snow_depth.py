...
def simple_cleanup(data,mask):
	...
	for i in np.arange(len(data) - 1) + 1:
		if (min_condition > data[i] > max_condition) | (np.abs(data[i]-data[i-1]) > 15):
			data[i]=data[i-1]
			mask[i]=0
...
def smooth_snow(snow):
	...
	for i in np.arange(sz[0]-windowsize)+halfwin:
		for j in range(sz[1]):
			snow[i,j]=np.median(snow[i-halfwin:i+halfwin,j])
...
def main(datafile, keyfile, calfile,badfile,inches=False,Farenheight=False,Chimney_2011=False):
	...
	for i in range(len(keys[:,0])):
		...
		if keys[i,4]>0:
			...
			else: airt=data[:,keys[i,4]]+273.15
			if ~Chimney_2011: snow*=sqrt(airt/273.15)
		if inches & ~Chimney_2011:
			print("Working in Inches")
			snow*=2.54

		if Chimney_2011:
			print("Correcting Chimney Park")
			snow=(snow)*25.4 # "snow" is actually time
			snow=snow*0.3314/2 #0.3314 cm/microsecond = speed of sound in air at 0C (air temperature correction follows)
