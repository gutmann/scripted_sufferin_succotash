#!/usr/bin/env python
# Copyright 2015 Ethan Gutmann, National Center for Atmospheric Research
...
# required columns in the tracker output file
datecol=0
timecol=1
latcol=2
loncol=3
vmax_col=7
r_column=16
# number of lines of header to skip over in the tracker output file
headerlength=1


# Haversine method to calculate distance between two lat/lon points
# given by lons, lats not tested, may need to switch to radians
...
def main(ens_member):
    """docstring for main"""
    files=glob.glob(input_dir.format(ens_member)+"track-*.txt")
    ...
    with open(output_file,"w") as output:
        for f in files:
            ...
            data=np.loadtxt(f,skiprows=headerlength,dtype='d')
